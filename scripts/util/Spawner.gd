...
func handle_body_as_spawned(spawning_body:Node2D):
	...
	spawning_body.global_position = global_position + (spawn_velocity_direction * spawn_dist)
	
	# initialize velocity, if possible
	var spawning_body_as_rigidbody:RigidBody2D = spawning_body as RigidBody2D
	if spawning_body_as_rigidbody != null:
		var spawn_velocity_power:float = randf_range(min_initial_velocity, max_initial_velocity)
		spawning_body_as_rigidbody.set_axis_velocity(spawn_velocity_direction * spawn_velocity_power)
...
func spawn(spawn_owner:Node2D):
	...
	for i in num_to_spawn:
		...
		handle_body_as_spawned(spawned_instance)
