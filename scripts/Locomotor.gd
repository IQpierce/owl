...
#TODO sam how do I get a sibling node?
@onready var thrust = get_node_or_null("../Thrust")
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("thrust"):
		...
		if body.linear_velocity.length_squared() > 0:
			...
			var potential_velocity:Vector2 = body.linear_velocity + thrust_para_vel
		...
		if Input.is_action_just_pressed("thrust"):
			if time_now - thrust_tap_time <= double_tap_msec && acceleration.dot(body.linear_velocity) < 0:
				acceleration *= turn_around_acceleration_factor;
				thrust_tap_time = 0
			thrust_tap_time = time_now

		body.apply_central_impulse(acceleration * thrust_delta)
...
#todo could turning around be faster, like if you are turning against your velocity you turn faster
	...
	#if body.linear_velocity.length_squared() < 0.000001:
